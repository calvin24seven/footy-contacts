      ...
      {open && (
        <>
          ...
          {/* Drawer
              ...
              Desktop : inline card below the filter row, height-bounded with internal scroll */}
          <div className="
            fixed inset-0 z-50 flex flex-col bg-navy overflow-hidden
            md:relative md:inset-auto md:z-auto md:flex md:flex-col md:overflow-hidden
            md:mt-3 md:rounded-xl md:border md:border-gray-600/50 md:bg-navy-light
            md:max-h-[min(640px,calc(100vh-200px))]
          ">
            {/* Mobile-only header */}
            ...
            {/* Scrollable content */}
            <div className="flex-1 overflow-y-auto overscroll-contain p-4 pb-2 md:p-5 md:pb-3 space-y-5">

              {/* ── Role & Category ─────────────────────────────────────── */}
              <div className="space-y-3">
                <p className="text-[10px] font-semibold text-gray-500 uppercase tracking-wider">Role & Category</p>
                <ChipInput
                  label="Role includes"
                  values={roleValues}
              ...
              <div className="space-y-3">
                <p className="text-[10px] font-semibold text-gray-500 uppercase tracking-wider">Organisation & Location</p>
                <ChipInput
              ...
              <div className="space-y-3 pb-2">
                <p className="text-[10px] font-semibold text-gray-500 uppercase tracking-wider">Data quality</p>
                <div>
            ...
            {/* Sticky action footer — always visible on both mobile and desktop */}
            <div className="shrink-0 px-4 py-3 border-t border-gray-700/60 flex items-center gap-2 bg-navy md:bg-navy-light md:px-5">
              <button
                onClick={applyFilters}
                disabled={isPending}
                className="flex-1 md:flex-none px-6 py-2.5 bg-gold text-navy rounded-lg text-sm font-semibold hover:bg-gold-dark disabled:opacity-50 transition-colors cursor-pointer disabled:cursor-not-allowed"
              >
                {isPending ? "Applying…" : "Apply filters"}
              </button>
              <button
                onClick={clearAll}
                className="px-4 py-2.5 text-sm text-gray-400 hover:text-white border border-gray-700 hover:border-gray-500 rounded-lg transition-colors cursor-pointer"
              >
                Clear
              </button>
              <button
                onClick={() => setOpen(false)}
                className="px-4 py-2.5 text-sm text-gray-500 hover:text-white transition-colors md:ml-auto cursor-pointer"
              >
                Cancel
              </button>
            </div>
