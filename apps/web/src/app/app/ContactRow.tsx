...
function MobileIdentity({
  ...
  return (
    <>
      ...
      {showSignals && (contact.has_email || contact.has_phone || contact.has_linkedin) && (
...
export default function ContactRow({
  ...
  return (
    <>
      ...
      <div
        className={cn(
          "md:hidden flex items-start gap-3 px-4 py-3.5",
          rowBase,
          isSelected ? rowSelected : rowIdle,
        )}
      >
        ...
        <div className="flex-1 min-w-0">
          {onPreview ? (
            <button className="w-full text-left" onClick={() => onPreview(contact)}>
              <MobileIdentity contact={contact} location={location} showSignals={false} />
            </button>
          ) : (
          ...
          {/* Signals + CTA row — below the tappable name/role/org area */}
          <div className="flex items-center mt-1.5" onClick={(e) => e.stopPropagation()}>
            ...
            {/* w-20 gives w-full on the button a definite parent → no overflow */}
